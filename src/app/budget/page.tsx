...
export default function BudgetPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100 flex items-center">
            <BarChart3 className="w-8 h-8 mr-3 text-emerald-500" aria-hidden="true" />
      ...
      {/* Tabs */}
      <div className="flex space-x-1 mb-6 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg w-fit">
        <button
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 mb-6">
        <div className="p-4 border-b border-slate-200 dark:border-slate-700">
          {activeTab === 'budgets' ? (
            <>
              <h2 className="font-semibold text-slate-900 dark:text-slate-100">Annual Operating & Capital Budgets</h2>
            ...
            <>
              <h2 className="font-semibold text-slate-900 dark:text-slate-100">Annual Comprehensive Financial Reports</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
        ...
        <div className="p-4">
          {loading ? (
            <div className="text-center py-12" role="status" aria-live="polite">
              ...
              <p className="text-slate-500 dark:text-slate-400 mt-4">Loading financial data...</p>
            </div>
          ) : activeTab === 'budgets' ? (
            budgetSummaries.length > 0 ? (
              <div className="space-y-3">
                {budgetSummaries.map((item) => (
                  <div key={`budget-${item.fiscalYear}`} className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                    <button
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        <span className="font-medium text-slate-900 dark:text-slate-100">{item.fiscalYear} Budget</span>
            ...
            auditSummaries.length > 0 ? (
              <div className="space-y-3">
                {auditSummaries.map((item) => (
                  <div key={`audit-${item.fiscalYear}`} className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                    <button
      ...
      <div className="bg-blue-50 dark:bg-blue-900/30 rounded-xl p-6 border border-blue-200 dark:border-blue-800">
        <h3 className="font-semibold text-blue-900 dark:text-blue-100 mb-2">Understanding City Financial Documents</h3>
